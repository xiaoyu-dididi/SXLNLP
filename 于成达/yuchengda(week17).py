...
def item_cf(user_id, item_id, similar_items, user_to_rating):
    pre_score = 0
    count = 0
    for similar_item, similar_socre in similar_items[item_id][:10]:
        # 该用户对与相似的电影的打分
        rating_by_similar_user = user_to_rating[user_id][similar_item - 1]
        # 分数加权
        pre_score += rating_by_similar_user * similar_socre
        if rating_by_similar_user != 0:
            count += 1
    pre_score /= count + 1e-5
    return pre_score
...
def movie_recommand(user_id, similar_user, similar_items, user_to_rating, item_to_name, topn=10):
    ...
    for item_id in unseen_items:
        #user_cf打分
        score1 = user_cf(user_id, item_id, similar_user, user_to_rating)
        # item_cf打分
        score2 = item_cf(user_id, item_id, similar_items, user_to_rating)
...
if __name__ == "__main__":
    user_item_score_data_path = r"E:\badouai\ai\第十七周 推荐系统\week17 推荐系统\ml-100k\u.data"
    item_name_data_path = r"E:\badouai\ai\第十七周 推荐系统\week17 推荐系统\ml-100k\u.item"
